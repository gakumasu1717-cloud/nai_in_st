 ...
 * NaiStudio - 프롬프트 헬퍼
 *
 *  - 와일드카드   : __name__ (설정에 저장된 목록에서 랜덤 1줄)
 *  - 인라인 랜덤  : {a|b|c}  /  {2$$a|b|c} (2개 뽑기)
...
/**
 * {a|b|c} 및 {2$$a|b|c} 전개. 중첩도 처리한다.
 */
export function expandInlineRandom(text) {
    let result = String(text ?? '');
    let guard = 0;

    while (/\{[^{}]*\|[^{}]*\}/.test(result) && guard++ < 50) {
        result = result.replace(/\{([^{}]*)\}/g, (match, body) => {
            if (!body.includes('|')) return match;

            let count = 1;
            let content = body;
            const countMatch = body.match(/^\s*(\d+)\s*\$\$\s*(.*)$/s);
            if (countMatch) {
                count = Math.max(1, Number(countMatch[1]));
                content = countMatch[2];
            }

            const options = content.split('|').map(s => s.trim()).filter(Boolean);
            if (options.length === 0) return '';
            if (count === 1) return pickRandom(options);
 ...
 * __wildcard__ 전개.
 * @param {string} text
 * @param {Record<string, string[]>} wildcards
 */
export function expandWildcards(text, wildcards) {
    let result = String(text ?? '');
    let guard = 0;

    while (/__([\w가-힣 .-]+)__/.test(result) && guard++ < 20) {
        result = result.replace(/__([\w가-힣 .-]+)__/g, (match, name) => {
            const list = wildcards?.[name] ?? wildcards?.[name.toLowerCase()];
            if (!Array.isArray(list) || list.length === 0) return match;
...
/** 생성 직전 최종 전개 (와일드카드 → 인라인 랜덤 → 정규화) */
export function resolvePrompt(text, wildcards, { normalize = true } = {}) {
    const expanded = expandInlineRandom(expandWildcards(text, wildcards));
    return normalize ? normalizeTags(expanded) : expanded.trim();
}
