 ...
 *
 * 최종 산출물은 StyleStudio 내부 포맷(normalized)으로 통일한다.
 */
...
export async function readStealthMetadata(blob) {
    try {
        ...
        console.debug('[StyleStudio] stealth 메타데이터 추출 실패:', error);
        return null;
    }
}
...
export async function extractImageMetadata(blob) {
    ...
    if (chunks.Comment) {
        try {
            return fromNaiComment(JSON.parse(chunks.Comment), chunks);
        } catch (_) {
            // Comment가 JSON이 아니면 Description만이라도 사용
            if (chunks.Description) {
                return { ...EMPTY_RESULT, found: true, kind: 'novelai', prompt: chunks.Description.trim(), raw: chunks };
            }
        }
    }

    // 2) A1111 / ComfyUI
    if (chunks.parameters) return fromA1111(chunks.parameters);
    if (chunks.prompt || chunks.workflow) {
        const comfy = fromComfy(chunks.prompt || chunks.workflow);
        if (comfy.found) return comfy;
    }
    ...
    if (stealth) {
        if (stealth.Comment) {
            try {
                return fromNaiComment(JSON.parse(stealth.Comment), stealth);
            } catch (_) {
                return { ...EMPTY_RESULT, found: true, kind: 'novelai', prompt: String(stealth.Comment).trim(), raw: stealth };
            }
        }
        if (stealth.Description) {
            return { ...EMPTY_RESULT, found: true, kind: 'novelai', prompt: String(stealth.Description).trim(), raw: stealth };
        }
    }

    return { ...EMPTY_RESULT, raw: chunks };
}
