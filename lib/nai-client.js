/**
 * StyleStudio - 이미지 생성 클라이언트
 *
 * 백엔드 3단 폴백:
 *   1) stylestudio 서버 플러그인 : 캐릭터 프롬프트 + 바이브 + 레퍼런스 + cfg_rescale 전부 지원
...
const ENDPOINTS = {
    [BACKEND.OWN]: '/api/plugins/stylestudio/generate-image',
...
export async function pingOwnPlugin() {
    try {
        const response = await fetch('/api/plugins/stylestudio/ping', { headers: getRequestHeaders() });
        availability[BACKEND.OWN] = response.ok;
        return response.ok;
    } catch (_) {
...
export async function letterboxReference(base64) {
    try {
        ...
        console.warn('[StyleStudio] 레퍼런스 레터박싱 실패, 원본 사용:', error);
        return base64;
    }
}
 ...
 * @param {object} state StyleStudio 생성 상태
 */
 ...
 * 서버 엔드포인트에는 없다. StyleStudio는 SD 확장을 거치지 않으므로
 * 여기서 직접 같은 일을 해야 한다.
 */
...
export async function generateImage(payload, { preferred = 'auto', onBackendChange } = {}) {
    ...
    for (const backend of order) {
        ...
        try {
            ...
            const headerSeed = response.headers?.get?.('X-StyleStudio-Seed')
 ...
 */
export async function saveImageToServer(base64, filenameHint = 'stylestudio', folder = 'StyleStudio') {
...
export async function sendImageToChat(imagePath, title) {
    ...
    const context = getContext();
    const message = {
        name: context.name2 || 'StyleStudio',
        is_user: false,
        is_system: false,
        send_date: Date.now(),
        mes: title || '[StyleStudio] 생성 이미지',
        extra: {
            image: imagePath,
            title: title || '',
            inline_image: false,
            generationType: 'stylestudio',
        },
    };
