/**
 * StyleStudio - 설정/그림체 저장소
 *
 * 모든 데이터는 extension_settings['StyleStudio'] 에 들어가며 ST 설정과 함께 서버에 저장된다.
...
export const EXTENSION_NAME = 'StyleStudio';
...
const DEFAULT_SETTINGS = {
    ...
    styles: [],
    wildcards: {
        'hair': ['long black hair', 'short blonde hair', 'silver twintails', 'wavy brown hair'],
        'lighting': ['cinematic lighting', 'soft lighting', 'backlighting', 'rim light'],
    },
    history: [],
...
export function exportStyles(ids = null) {
    ...
    return JSON.stringify({ type: 'stylestudio-styles', version: SETTINGS_VERSION, styles }, null, 2);
}
...
export function getWildcards() {
    return getSettings().wildcards ?? {};
}

export function setWildcards(map) {
    getSettings().wildcards = map;
    save();
}
